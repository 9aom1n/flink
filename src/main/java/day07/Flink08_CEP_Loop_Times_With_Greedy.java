...
public class Flink08_CEP_Loop_Times_With_Greedy {
    public static void main(String[] args) throws Exception {
        ...
        //获取文件中的数据
        DataStreamSource<String> dataStreamSource = environment.readTextFile("input\\sensor1.txt");
        ...
        Pattern<WaterSensor, WaterSensor> begin = Pattern.<WaterSensor>begin("begin").where(new IterativeCondition<WaterSensor>() {
            ...
            public boolean filter(WaterSensor waterSensor, Context<WaterSensor> context) throws Exception {
                ...
                .greedy()      //贪婪模式 : 在组合模式情况下，对次数的处理尽快能获取最多个的那个次数，就是贪婪，当一个时间同时满足两个模式的时候起作用。
                               // todo 注意： 1.一半贪婪比非贪婪的结果要少
                               //            2.模式组不能设置为贪婪
                .next("next")
